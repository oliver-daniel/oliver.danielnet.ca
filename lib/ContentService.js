...
export default class ContentService {
  ...
  static getPublishedProjectURIs() {
    const projects = Object.values(this.getAllProjects()).flat(1);

    return projects.reduce((acc, { id, data }) => {
      id = id.split("/").at(-1);
      if (data.published) return acc.concat(id);
      return acc;
    }, []);
  }
}
